...
def get_triggers(label):
    return zapi.trigger.get(host="ru1-aruba-mc01",output="triggerid",search={"description": "Access Point "+label},filter={"status": "0"})
def get_mac(label):
    return zapi.item.get(host="ru1-aruba-mc01", output=["itemid","lastvalue"], application="Hardware", search={"name": "*" + label + "*Mac"}, searchWildcardsEnabled="true")
def get_channels(label):
    chan1 = zapi.item.get(host="ru1-aruba-mc01", output=["itemid", "lastvalue"], application="Hardware", search={"name": "*" + label + "*channel1"}, searchWildcardsEnabled="true")
    chan2 = zapi.item.get(host="ru1-aruba-mc01", output=["itemid", "lastvalue"], application="Hardware", search={"name": "*" + label + "*channel2"}, searchWildcardsEnabled="true")
    if chan1 and chan2:
